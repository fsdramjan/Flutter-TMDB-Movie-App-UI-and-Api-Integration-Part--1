final movieImageAndName = [
  {
    'id': 1,
    'title': 'Ghayal ',
    'category': 'Romantic',
    'image':
        'https://static-koimoi.akamaized.net/wp-content/new-galleries/2015/11/ghayal-once-again-movie-poster-3.jpg',
    'year': '2019',
    'desciption':
        '''Businessman and gold smuggler George Peter and his driver are killed under mysterious circumstances. Police investigators Muhammed Easa and Isaac discover that it was carried out with a surgical blade, causing them to think a doctor was behind it. They start investigating medical shops. At the same time, Marco JR, George's brother, meets Davis and his sons – their rivals – to ask whether it was their work. Davis's son Abraham starts speaking in a condescending manner, enraging Marco and provoking him to shoot Abraham.During his funeral, Easa asks Davis if it was Marco. Davis does not tell him, thinking that he could seek revenge on his own.Some days after an unknown person shoots Patrick – the mid stander between the 2 gangs – Easa and Isaac take him to Star Hospital where he gets murdered despite being protected with maximum security. This solidifies Easa and Isaac's theory that the murderer is a doctor .'''
  },
  {
    'id': 2,
    'title': 'Bhuj',
    'category': 'Action',
    'image':
        'https://imgcacheblog.instube.com/2020/09/Bhuj-The-Pride-of-India-Full-Movie-Download-InsTube.jpg',
    'year': '2020',
    'desciption':
        '''As a patriotic film, Bhuj: The Pride of India (2020) got Abhishek Dudhaiya as the director while Ajay Devgn, Sanjay Dutt, and Sonakshi Sinha as the leading roles.

In short, the Hindi war film tells a story about an IAF squadron leader who is in charge of the Bhuj airbase and reconstruct the runway with the help of 300 local women.

So, if you are going to watch this Hindi war action drama, InsTube will tell you more about Bhuj The Pride of India full movie:'''
  },
  {
    'id': 3,
    'title': 'I Love Aaj Kaal ',
    'category': 'New Movie',
    'image': 'https://i.ytimg.com/vi/EZ8lTqDmSyQ/maxresdefault.jpg',
    'year': '2018',
    'desciption':
        '''The story that comes after two lovers break up when they realize that their career goals are pulling them in different directions.'''
  },
  {
    'id': 4,
    'title': 'Cocktail ',
    'category': 'Trending',
    'image': 'https://i.ytimg.com/vi/H1C_EiI3Fe0/maxresdefault.jpg',
    'year': '2019',
    'desciption':
        '''Cocktail is a Hindi rom-com movie based on a love triangle between Gautam (Saif Ali Khan), Veronica (Deepika Padukone) & Meera (Diana Penty). The movie shows the bond between three friends who meet in London. All is well as Gautam & Veronica instantly click but things start to get bumpy as Meera & Gautam fall for each other. Watch Cocktail to see who Gautam chooses between Veronica and Meera .'''
  },
  {
    'id': 5,
    'title': 'MIKHEAL KA BHAUKAL',
    'category': 'Hot',
    'image': 'https://i.ytimg.com/vi/5-peXNX-V6c/maxresdefault.jpg',
    'year': '2018',
    'desciption':
        '''Businessman and gold smuggler George Peter and his driver are killed under mysterious circumstances. Police investigators Muhammed Easa and Isaac discover that it was carried out with a surgical blade, causing them to think a doctor was behind it. They start investigating medical shops. At the same time, Marco JR, George's brother, meets Davis and his sons – their rivals – to ask whether it was their work. Davis's son Abraham starts speaking in a condescending manner, enraging Marco and provoking him to shoot Abraham.During his funeral, Easa asks Davis if it was Marco. Davis does not tell him, thinking that he could seek revenge on his own.Some days after an unknown person shoots Patrick – the mid stander between the 2 gangs – Easa and Isaac take him to Star Hospital where he gets murdered despite being protected with maximum security. This solidifies Easa and Isaac's theory that the murderer is a doctor.'''
  },
  {
    'id': 6,
    'title': 'Puspa ',
    'category': 'Popular',
    'image':
        'https://www.quirkybyte.com/wp-content/uploads/2022/01/Pushpa-Full-Movie-in-Hindi-Download-filmyzilla.jpg',
    'year': '2021',
    'desciption':
        '''Pushpa Full Movie In Hindi Download Bolly4u – Pushpa, Full Name: Pushpa: The Rise – Part 1, is an Indian Telugu box office action thriller that was both written and directed by Sukumar.'''
  },
];
